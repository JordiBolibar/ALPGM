...
ANALYSIS OF MB PROJECTIONS USING A STATIC GEOMETRY TO INVESTIGATE THE EFFECTS
OF NONLINEARITIES ON FLAT GLACIERS AND ICE CAPS
...
load_dictionaries = True  # True -> make plots / False -> create netCDF datasets
other_plots = False
static_flat_comparison = False
#filter_member = False
# mer de glace
...
if(lasso):
    path_glacier_evolution = os.path.join(workspace, 'glacier_data', 'glacier_evolution', 'static_geometry_lasso')
    path_MB = os.path.join(workspace, 'glacier_data', 'SMB') 
    path_MB_simulations = os.path.join(path_MB, 'SMB_simulations', 'static_geometry_lasso')
else:
    path_glacier_evolution = os.path.join(workspace, 'glacier_data', 'glacier_evolution', 'static_geometry')
    path_MB = os.path.join(workspace, 'glacier_data', 'SMB') 
    path_MB_simulations = os.path.join(path_MB, 'SMB_simulations', 'static_geometry')
...
proj_blob = {'year':[], 'data':[]}
annual_mean = {'MB':copy.deepcopy(proj_blob), 
...
raw_members = os.listdir(os.path.join(path_glacier_s_CPDDs, "projections"))
raw_IDs = os.listdir(os.path.join(path_glacier_s_CPDDs, "projections", "FORCING_CLMcom-CCLM4-8-17_CNRM-CERFACS-CNRM-CM5_RCP45_alp_2005080106_2100080106", "1"))
...
for ID in raw_IDs:
    if(not np.any(glims_IDs == ID[:-16])):
        glims_IDs = np.append(glims_IDs, ID[:-16])
...
glacier_projections_dict = {'MB':copy.deepcopy(dummy_coords), 
...
def save_plot_as_pdf(fig, variables, with_26, lasso):
    if(lasso):
        model = "lasso"
    else:
        model = "DL"
    # Save as PDF
    if(with_26):
        fig.savefig(os.path.join(path_glacier_evolution_plots, 'summary', 'pdf', 'glacier_' + str(variables) + '_evolution_' + model + '_with_26.pdf'))
        fig.savefig(os.path.join(path_glacier_evolution_plots, 'summary', 'jpeg', 'glacier_' + str(variables) + '_evolution_' + model + '_with_26.jpeg'))
    else:
        fig.savefig(os.path.join(path_glacier_evolution_plots, 'summary', 'pdf', 'glacier_' + str(variables) + '_evolution_' + model + '.pdf'))
        fig.savefig(os.path.join(path_glacier_evolution_plots, 'summary', 'jpeg', 'glacier_' + str(variables) + '_evolution_' + model + '.jpeg'))
...
def store_RCP_mean(path_variable, variable, RCP_means):
    ...
    for RCP in RCPs:
        if((with_26 and RCP == '26') or RCP != '26'):
            ...
            if(with_26):
                np.savetxt(os.path.join(path_RCP_means, "RCP" + str(RCP) + "_" + model + "_glacier_with_26_" + str(variable) + "_" + str(years[0])+ "_" + str(years[-1]) + '.csv'), 
                           data_years, delimiter=";", fmt="%s")
                
                config_dictionary_file = os.path.join(path_RCP_means, "RCP" + str(RCP) + "_" + model + "_glacier_with_26_" + str(variable) + "_" + str(years[0])+ "_" + str(years[-1]))
                with open('config.dictionary', 'wb') as config_dictionary_file:
                    pickle.dump(RCP_means, config_dictionary_file)
            else:
                np.savetxt(os.path.join(path_RCP_means, "RCP" + str(RCP) + "_" + model + "_glacier_" + str(variable) + "_" + str(years[0])+ "_" + str(years[-1]) + '.csv'), 
                           data_years, delimiter=";", fmt="%s")
                
                config_dictionary_file = os.path.join(path_RCP_means, "RCP" + str(RCP) + "_" + model + "_glacier_" + str(variable) + "_" + str(years[0])+ "_" + str(years[-1]))
                with open('config.dictionary', 'wb') as config_dictionary_file:
                    pickle.dump(RCP_means, config_dictionary_file)
...
if(not load_dictionaries):
    ...
    first = True
    root_paths = zip(path_MB_root, 
                     path_s_CPDDs_root, path_w_CPDDs_root,
                     path_s_snowfall_root, path_w_snowfall_root,
                     path_s_rain_root, path_w_rain_root)
    for path_forcing_MB, path_forcing_s_CPDDs, path_forcing_w_CPDDs, path_forcing_s_snowfall, path_forcing_w_snowfall, path_forcing_s_rain, path_forcing_w_rain in root_paths:
        
        current_RCP = path_forcing_s_CPDDs[-28:-26]
        current_member = path_forcing_s_CPDDs[8:-32]
...
#        if((with_26 and np.any(current_member == members_with_26)) or (not with_26 and current_RCP != '26')):
    ...
    #    if(current_member == 'CLMcom-CCLM4-8-17_CNRM-CERFACS-CNRM-CM5'):
            print("\nProcessing " + str(path_forcing_s_CPDDs))
    ...
    #        print("member_idx: " + str(member_idx))
            ...
            volume_scale_paths = zip(path_MB_glaciers,
            ...
            for path_MB_scaled, path_s_CPDDs_scaled, path_w_CPDDs_scaled, path_s_snowfall_scaled, path_w_snowfall_scaled, path_s_rain_scaled, path_w_rain_scaled in volume_scale_paths:
    ...
    #            if(path_area_scaled == '1'):
                if(path_s_CPDDs_scaled == '1' and path_s_CPDDs_glaciers_scaled.size > 369):
                    bump_member = True
                    glacier_paths = zip(path_MB_glaciers_scaled, 
                    ...
                    for path_MB, path_s_CPDD, path_w_CPDD, path_s_snowfall, path_w_snowfall, path_s_rain, path_w_rain in glacier_paths:
    ...
    #                            print("\nmember_idx: " + str(member_idx))
                                years_path = zip(MB_glacier, 
                                                 s_CPDD_glacier, w_CPDD_glacier,
                                                 s_snowfall_glacier, w_snowfall_glacier,
                                                 s_rain_glacier, w_rain_glacier)
                                
                                year=year_start
                                for MB_y, s_CPDD_y, w_CPDD_y, s_snowfall_y, w_snowfall_y, s_rain_y, w_rain_y in years_path:
...
if(not load_dictionaries):    
    ...
    if(lasso):
        ds_glacier_projections_lasso.to_netcdf(os.path.join(path_glacier_evolution, 'glacier_evolution_lasso_static_' + str(year_start) + '_2100.nc'), encoding=encoding)
    else:
        ds_glacier_projections_lasso.to_netcdf(os.path.join(path_glacier_evolution, 'glacier_evolution_static_' + str(year_start) + '_2100.nc'), encoding=encoding)
    ...
    if(lasso):
        df_glacier_projections.to_csv(os.path.join(path_glacier_evolution, 'glacier_evolution_lasso_static' + str(year_start) + '_2100.csv'), sep=";")
    else:
        df_glacier_projections.to_csv(os.path.join(path_glacier_evolution, 'glacier_evolution_static' + str(year_start) + '_2100.csv'), sep=";")
    ...
    path_glacier_evolution_lasso = os.path.join(workspace, 'glacier_data', 'glacier_evolution', 'static_geometry_lasso')
    ds_glacier_projections_lasso = xr.open_dataset(os.path.join(path_glacier_evolution_lasso, 'glacier_evolution_lasso_static_' + str(year_start) + '_2100.nc'))
    
    path_glacier_evolution_nn = os.path.join(workspace, 'glacier_data', 'glacier_evolution', 'static_geometry')
    ds_glacier_projections = xr.open_dataset(os.path.join(path_glacier_evolution_nn, 'glacier_evolution_static_' + str(year_start) + '_2100.nc'))
...
#    algpm_mean_MB_03_15 = ds_smb_reconstructions.sel(RGI_ID=RGI_ID_sel).mean('RGI_ID')[-12:].mean()
...
#    ax1[7].set_ylim([-18,10])
    ...
    ### UNCOMMENT TO OBTAIN PDFs BASED ON ALL PROJECTED VALUES INSTEAD OF RCP MEANS
    
#    MB_26_nn_all = ds_glacier_projections.MB.sel(RCP='26', member=submembers).values.flatten()
#    MB_26_nn_all = MB_26_nn_all[~np.isnan(MB_26_nn_all)]
#    MB_45_nn_all = ds_glacier_projections.MB.sel(RCP='45', member=submembers).values.flatten()
#    MB_45_nn_all = MB_45_nn_all[~np.isnan(MB_45_nn_all)]
#    MB_85_nn_all = ds_glacier_projections.MB.sel(RCP='85', member=submembers).values.flatten()
#    MB_85_nn_all = MB_85_nn_all[~np.isnan(MB_85_nn_all)]
#    MB_26_lasso_all = ds_glacier_projections_lasso.MB.sel(RCP='26', member=submembers).values.flatten()
#    MB_26_lasso_all = MB_26_lasso_all[~np.isnan(MB_26_lasso_all)]
#    MB_45_lasso_all = ds_glacier_projections_lasso.MB.sel(RCP='45', member=submembers).values.flatten()
#    MB_45_lasso_all = MB_45_lasso_all[~np.isnan(MB_45_lasso_all)]
#    MB_85_lasso_all = ds_glacier_projections_lasso.MB.sel(RCP='85', member=submembers).values.flatten()
#    MB_85_lasso_all = MB_85_lasso_all[~np.isnan(MB_85_lasso_all)]
#    
##    import pdb; pdb.set_trace()
#    
    bins = 'auto'
    ...
    kde_x_lasso_26 = np.linspace(MB_26_lasso.values.min(), MB_26_lasso.max(), 301)
...
#    
    ...
    fig1.savefig('C:\\Jordi\\PhD\\Publications\\Third article\\Nature Communications\\Figures\\DL_vs_Lasso_static.pdf')
...
if(other_plots):
    ...
    # Save as PDF
    save_plot_as_pdf(fig1, header + 'mb_volume_area', with_26, lasso)
    ...
    # Save as PDF
    save_plot_as_pdf(fig3, header + 'CPDD_snowfall_rain', with_26, lasso)
...
#    store_RCP_mean(path_glacier_CPDDs, 'CPDD', RCP_means)
    ...
    # Save as PDF
    save_plot_as_pdf(fig5, header + 'meltwater_discharge', with_26, lasso)
    ...
    #        fig6.suptitle("Glacier retreat topographical feedback on climate projections")
        ...
        # Save as PDF
        save_plot_as_pdf(fig6, header + 'static_vs_dynamical_CPDD_snowfall_rain', with_26, lasso)
    ...
    #        fig7.suptitle("Glacier retreat topographical feedback on glacier-wide MB projections")
        ...
        # Save as PDF
        save_plot_as_pdf(fig7, header + 'static_MB', with_26, lasso)
